    ...
    const selected = body.data.ids
      ? body.data.ids.length
        ? await db
            .select()
            .from(items)
            .where(inArray(items.id, body.data.ids))
            .limit(40)
        : []
      : await db
          .select()
          .from(items)
          .where(body.data.type ? eq(items.type, body.data.type) : undefined)
          .limit(40)
    const requested = body.data.ids ? new Set(body.data.ids) : undefined
    const selectedItems = requested
      ? selected.filter((item) => requested.has(item.id))
      : selected
    ...
    for (const item of selectedItems) {
      try {
        ...
        failed.push({
          ...
          slug: item.slug,
          reason:
            cause instanceof Error ? cause.message : "Provider sync failed.",
        })
      }
    }
