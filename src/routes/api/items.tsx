...
import { itemExists, lookupCollection, uniqueSlug } from "@/server/items"
import { storeCover } from "@/server/covers"
import { items, itemTypes } from "@/server/schema"

const unauthorized = () => Response.json({ error: "Unauthorized" }, { status: 401 })
const output = { id: items.id, slug: items.slug, type: items.type, status: items.status, title: items.title, creator: items.creator, year: items.year, format: items.format, edition: items.edition, tmdbId: items.tmdbId, openLibraryKey: items.openLibraryKey, coverImageUrl: items.coverImageUrl }
    ...
    POST: async ({ request }) => {
      ...
      for (const input of body.data.items) {
        try {
          ...
          if (input.type === "book" && input.edition) { failed.push({ query: input.query, reason: "Only movies and TV shows can have an edition." }); continue }
          if (await itemExists({ type: input.type, title: top.title, year: top.year ?? 0, providerId, edition: input.edition })) {
            skipped.push({ query: input.query, reason: "Already on Shelf" }); continue
          }
          const resolved = { ...top, slug: await uniqueSlug(slugify(top.title), input.edition) }
          if (body.data.dryRun) { added.push({ title: resolved.title, slug: resolved.slug }); continue }
          const now = new Date().toISOString()
          const [created] = await db.insert(items).values({ slug: resolved.slug, type: input.type, status: input.status || "owned", title: resolved.title, creator: resolved.creator, year: resolved.year ?? 0, format: input.format || null, edition: input.edition || null, coverImageUrl: (await storeCover(resolved.coverImageUrl, resolved.slug)) || null, tmdbId: input.type === "book" ? null : providerId, openLibraryKey: input.type === "book" ? providerId : null, notes: "", createdAt: now, updatedAt: now }).returning({ id: items.id, title: items.title, slug: items.slug })
          added.push(created)
